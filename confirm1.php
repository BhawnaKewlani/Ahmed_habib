...
function createRandomPassword() {
    ...
    srand((double)microtime()*1234567890987);
...
$result1 = mysql_query("SELECT * FROM reservation where confirmation='".$confirm."'");


while($row = mysql_fetch_array($result1))
  {
  $item_name=$row['room_id'];
  $item_number=$row['confirmation'];
  $amount=$row['payable'];

  $txn_id=createRandomPassword();
  $payer_email=$row['email'];
  }
$result2 = mysql_query("SELECT * FROM room where room_id='".$item_name."'");
...
if (!mysql_query($res,$link))
  ...
  {
  echo '<br /><font size="6"> Payment is Complete</font>';
  }
